...
impl Worker {
    ...
    pub async fn run(self, mut rx: mpsc::Receiver<PipelineCommand>) {
        while let Some(cmd) = rx.recv().await {
            match cmd {
                PipelineCommand::Append { stream_id, mut events, expected_version, resp_tx } => {
                    let res = self.handle_append(&stream_id, &mut events, expected_version).await;
    ...
    async fn handle_append(
        ...
        let current_events = self.store.fetch_stream(stream_id).await.map_err(|e| e.to_string())?;
        ...
        if expected_version != -1 && current_version != expected_version {
            tracing::warn!("Concurrency conflict for stream {}: expected {}, got {}", stream_id, expected_version, current_version);
            return Ok(false); 
        }
        ...
        for event in events.drain(..) {
            self.store.append_event(stream_id, event).await.map_err(|e| e.to_string())?;
        }
